...
export function ProductImport() {
  ...
  return <>
    <PageHead title="Advanced import" description="Paste an already approved product contract. Most people should use the guided product builder instead." />
    {!allowed ? <Alert>Only an organization or team administrator can add products.</Alert> : <form className="px-stack" onSubmit={submit} style={{ maxWidth: 760 }}>
      ...
      <Field label="Product address">{(f) => <Input id={f.id} required pattern="[a-z0-9][a-z0-9_-]{0,63}" value={productId} onChange={(e) => setProductId(e.target.value)} />}</Field>
